...
def _compute_similarity_matrix(
    ...
    if metric == "correlation":
        # Correlation can become NaN when a row has zero variance.
        # We compute it safely and define:
        # - similarity = 1 for pairs of constant vectors
        # - similarity = 0 for constant vs non-constant vectors
        X = features - features.mean(axis=1, keepdims=True)
        norms = np.linalg.norm(X, axis=1)

        numer = X @ X.T
        denom = np.outer(norms, norms)

        corr = np.divide(numer, denom, out=np.zeros_like(numer), where=denom > 0)

        zero = norms == 0
        if np.any(zero):
            corr[np.ix_(zero, zero)] = 1.0

        sim = np.clip(corr, 0.0, 1.0)
...
def compute_cooperbarahona(
    ...
    progress_cb: callable, optional
        ...
        Expected signature: progress_cb(progress: float, message: str)
    ...
    _progress(0.05, "Preparing nodes")
    nodes = list(G.nodes())

    _progress(0.15, "Computing structural signatures")
    if signature == "k-hop":
    ...
    _progress(0.50, "Computing similarity matrix")
    similarity = _compute_similarity_matrix(features, metric=similarity_metric)

    _progress(0.70, "Clustering similarity matrix")
...
def createadjmatrix(Ni, Nj, R):
    ...
    tempfinalm = np.concatenate(
        (np.concatenate((tempm2, tempm1.T)), np.concatenate((tempm1, tempm3))), axis=1
    )
    return tempfinalm
...
def rolesim_calc(
    ...
    _progress(0.01, "Initializing RoleSim scores")
    ...
    for k in range(1, maxiter):
        # Progress based on iteration count
        iter_progress = 0.02 + 0.96 * (k / max(1, maxiter - 1))
        _progress(iter_progress, f"RoleSim iteration {k}/{maxiter - 1}")
        ...
        if all(x < convtol for x in tempm.flatten()):
            ...
            _progress(1.00, "RoleSim converged")
            break
        if k % 5 == 0:
    ...
    _progress(1.00, "RoleSim finished")
    return scores[(len(scores) - 1)]


def rolesim_star_calc(
    G,
    beta=0.10,
    lambd=0.70,
    maxiter=100,
    ...
    for k in range(1, maxiter + 1):
        iter_progress = 0.02 + 0.96 * (k / max(1, maxiter))
        _progress(iter_progress, f"RoleSim* iteration {k}/{maxiter}")
        ...
        for i in range(n):
            for j in range(n):
                ...
                if (Nil * Njl - Ml) == 0:
                    ...
                    stemp[i, j] = (1 - beta) * (
                        lambd * (temp / (Nil + Njl - Ml)) + (1 - lambd) * (temp2 / (Nil * Njl - Ml))
                    ) + beta
        scores[k] = stemp
        tempm = np.abs(stemp - sprev)
...
def compute_rolesim(G, beta, maxiter, clustering_method, n_roles, centrality_table, progress_cb=None):
    nodes = list(G.nodes())
    similarity = rolesim_calc(G, beta, maxiter, progress_cb=progress_cb)
...
def compute_rolesim_star(G, beta, lambd, maxiter, clustering_method, n_roles, centrality_table, progress_cb=None):
    nodes = list(G.nodes())
    similarity = rolesim_star_calc(G, beta, lambd, maxiter, progress_cb=progress_cb)
    labels_list = _cluster_similarity_matrix(similarity, n_roles, method=clustering_method)
    # Build labels dictionary keyed by node
    labels = {node: labels_list[i] for i, node in enumerate(nodes)}
    # Compute summary statistics per role
...
def compute_rolx(G, n_roles, centrality_table, progress_cb=None):
    nodes = list(G.nodes())

    if progress_cb is not None:
        progress_cb(0.10, "Initializing RolX")
    ...
    if progress_cb is not None:
        progress_cb(1.00, "Finished RolX")
...
def compute_roles(
    ...
    if method == "Cooper and Barahona":
        return compute_cooperbarahona(
            G,
            info["signature"],
            info["k"],
            info["t"],
            info["similarity_metric"],
            info["clustering_method"],
            info["n_roles"],
            centralities,
            progress_cb=progress_cb,
        )
    elif method == "RoleSim":
        return compute_rolesim(
            G,
            info["beta"],
            info["maxiter"],
            info["clustering_method"],
